...
class QMIX:
    def __init__(self, args):
        ...
        if args.optimizer == "RMS":
            ...
            self.optimizer = torch.optim.Adam(self.eval_parameters, lr=args.lr/10)

        self.eval_hidden = None
        self.target_hidden = None
        # print(f'Init alg QMIX with {self.args.replay_alg}')
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):
        ...
        if self.args.replay_alg == 'Stab-ER':
            q_evals_prob = torch.nn.functional.softmax(q_evals, dim=-1)
            q_targets_prob = torch.nn.functional.softmax(q_targets, dim=-1)

            ISweight = (q_evals_prob.sum() / q_targets_prob.sum())
        ...
        if self.args.replay_alg == 'Stab-ER':
            loss = (masked_td_error ** 2).sum()*ISweight / mask.sum()
        else:
